...
### 基础防识别功能
- **ProtectionStrength 枚举**：定义三个强度等级（Light/Medium/Strong）
- 控制参数：edgeInterference, warpAmount, moireAmount, channelShift, jpegQuality
- 主要技术：
  - 字符随机变形（旋转、倾斜、缩放、透明度）
  - 弹性扭曲（`addElasticWarp`）
  - 摩尔纹叠加（`addMoireEffect`）
  - 色差效果（`addChromaticAberration`）

### 高级防识别增强（独立功能）
- **AdvancedProtectionStrength 枚举**：独立的三个强度等级（Light/Medium/Strong）
- 控制参数：adversarialNoiseAmplitude, textureOverlayAlpha, edgePerturbationOffset, channelNoiseAmplitude, localBlurProbability
- 主要技术：
  - 对抗性噪声（`applyAdversarialNoise` - Perlin 噪声）
  - 高频纹理叠加（`applyTextureOverlay` - 点阵、微线条、交叉网格）
  - 边缘扰动（`applyEdgePerturbation` - Sobel 边缘检测 + 随机偏移）
  - 颜色通道独立噪声（`applyChannelIndependentNoise`）
  - 局部模糊/锐化（`applyLocalBlurSharpen` - 高斯模糊 + Unsharp mask）

### UI 状态管理
- `selectedStrength`：基础防识别强度
- `antiRecognitionEnabled`：高级防识别增强开关
- `selectedAdvancedStrength`：高级防识别强度（仅在开关开启时显示和使用）
...
textToBitmap() 
  ...
  → [可选] addAntiRecognitionEnhancement()
```
...
### 性能优化
- 使用 `bitmap.getPixels()` 和 `bitmap.setPixels()` 批量操作
- 可分离高斯核（水平 + 垂直卷积）
- 区域化处理（模糊/锐化仅应用于选中区域）
- Perlin 噪声使用网格插值生成平滑噪声场

### 参数设计原则
- 所有高级防识别参数经过调优，确保人类可读性
- 对抗性噪声幅度 < 12，避免明显颗粒感
- 纹理叠加透明度 < 40，保持图案细微
- 边缘扰动偏移 ≤ 3px，避免明显变形
- 通道噪声幅度差异 < 10，避免色偏
...
### 添加新的防识别技术
1. 如果是基础技术，添加到 `ProtectionStrength` 枚举
2. 如果是高级技术，添加到 `AdvancedProtectionStrength` 枚举
3. 创建对应的辅助函数（如 `applyXxx(bitmap, strength, random)`）
4. 在主函数中调用（`textToBitmap` 或 `addAntiRecognitionEnhancement`）
5. 确保参数在三个强度等级下都有合理的值
...
### 修改图像处理逻辑
- 注意位图的可变性（使用 `copy(Bitmap.Config.ARGB_8888, true)`）
- 像素操作后必须调用 `setPixels()` 写回
- 随机数生成器使用固定种子以保证可重现性（如需要）
- Canvas 操作在所有像素操作之后进行
